...
IBport=4002 #7497 is the port for tws and 4002 is for the gateway
...
getData=function(){
  ...
  temp<-NULL
  tryCatch(
    temp<-NasdaqDataLink.datatable("SHARADAR/SF2", ticker = 'AAPL', filingdate.gte = start_date),
    warning = function(w) {
      temp<-NULL
    },
    error = function(e) {
    ...
    finally = print("The NASDAQ API works fine")
  )
  if(!is.null(temp)){
...
genIndicators=function(sym){
  ...
  if (lengthforscaling>0) {
    maxs <- apply(stock.xts[c(1:lengthforscaling),], 2, max,na.rm=TRUE)
    mins <- apply(stock.xts[c(1:lengthforscaling),], 2, min,na.rm=TRUE)
    scaled.xts<-tryCatch({
...
while (!done) {
  cat("\014")
  proceed<-OkToProceed(message)
  done<-ifelse(proceed=="Y"|proceed=="N",TRUE,FALSE)  # Commented out |proceed=="Y"
}  
...
# ******************* OPEN CONNECTION & PROCESS TRADES  ************************
  ...
  if(!is.null(tws)) {
    ...
    available<-as.numeric(acc[[1]]$AvailableFunds[1])/2
  ...
  twsDisconnect(tws)
    ...
    print("Aborting: Cannot connect to Interactive Brokers")
  }
}
else {
